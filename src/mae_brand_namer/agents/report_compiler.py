...
class ReportCompiler:
    ...
    async def compile_report(
        ...
        try:
            ...
            # Final check - ensure survey_simulation is included if we have data for it
            try:
                # Check if survey_simulation is present in the sections
                survey_in_report = any(section.get('title', '').lower() == 'survey simulation' for section in report.get('sections', []))
                
                if not survey_in_report:
                    logger.info("Survey simulation missing from report, attempting to inject it")
                    
                    # Try to query for the raw survey simulation output
                    query = f"""
                    SELECT llm_response
                    FROM survey_simulation_raw 
                    WHERE run_id = '{run_id}' 
                    ORDER BY created_at DESC
                    LIMIT 1
                    """
                    survey_raw_results = await self.execute_with_retry(query, {})
                    
                    if survey_raw_results and len(survey_raw_results) > 0:
                        survey_raw = survey_raw_results[0].get("llm_response")
                        if survey_raw:
                            try:
                                # Try to parse as JSON if possible
                                import json
                                survey_data = json.loads(survey_raw)
                                if "survey_simulation" in survey_data:
                                    # Create a survey section
                                    survey_section = {
                                        "title": "Survey Simulation",
                                        "content": {"summary": survey_data["survey_simulation"]}
                                    }
                                    
                                    # Find the right position to insert
                                    # Typically after competitor analysis or before recommendations
                                    insert_index = len(report['sections'])  # Default to end
                                    for i, section in enumerate(report['sections']):
                                        if section['title'].lower() == 'competitor analysis':
                                            insert_index = i + 1
                                            break
                                    
                                    # Insert the survey section
                                    report['sections'].insert(insert_index, survey_section)
                                    logger.info(f"Added Survey Simulation section at index {insert_index}")
                            except Exception as e:
                                logger.warning(f"Error processing survey raw data: {str(e)}")
            except Exception as e:
                logger.warning(f"Error injecting survey simulation: {str(e)}")
    ...
    async def _fetch_section_data(self, run_id: str, section_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if section_name == "executive_summary":
                ...
                # If we have survey data from the LLM in the state, add it to the section data
                try:
                    # Try to query for the raw survey simulation output generated by the LLM
                    query = f"""
                    SELECT llm_response
                    FROM survey_simulation_raw 
                    WHERE run_id = '{run_id}' 
                    ...
                    if survey_raw_results and len(survey_raw_results) > 0:
                        ...
                        if survey_raw:
                            try:
                                ...
                                survey_data = json.loads(survey_raw)
                                if "survey_simulation" in survey_data:
                                    section_data["survey_simulation"] = survey_data["survey_simulation"]
                                    logger.info("Added raw survey simulation content from database")
                            except:
                                # If not valid JSON, use as-is
                                section_data["survey_simulation"] = survey_raw
                                logger.info("Added raw survey simulation content as plain text")
                except Exception as e:
                    logger.warning(f"Error fetching raw survey simulation: {str(e)}")
                    # Continue without the raw data
    ...
    async def _generate_section_in_chunks(
        ...
        if section_name == "linguistic_analysis" and "linguistic_analyses" in section_data:
        ...
        elif section_name == "name_generation" and "name_generations" in section_data:
            # Process name generations in chunks, organizing by naming_category
            all_generations = section_data.get("name_generations", [])
            logger.info(f"Processing {len(all_generations)} brand names in chunks")
            
            # Group by naming_category if available
            category_groups = {}
            for name in all_generations:
                category = name.get("naming_category", "Uncategorized")
                if category not in category_groups:
                    category_groups[category] = []
                category_groups[category].append(name)
            
            logger.info(f"Grouped brand names into {len(category_groups)} categories: {list(category_groups.keys())}")
            
            summary_parts = []
            processed_names = []
            
            # Process each category as a separate chunk
            for category, names in category_groups.items():
                chunk_data = {
                    "name_generations": names,
                    "category": category,
                    "category_count": len(names),
                    "total_names": len(all_generations)
                }
                ...
                chunk_content = await self._generate_single_chunk(
                    run_id, 
                    section_name,
                    f"{section_title} ({category} Names)", 
                    chunk_data,
                    system_message
                )
                    ...
                    for key in chunk_content:
                        if key != "summary":
                            if key not in combined_content:
                                if isinstance(chunk_content[key], list):
                                    combined_content[key] = []
                                elif isinstance(chunk_content[key], dict):
                                    combined_content[key] = {}
                                else:
                                    combined_content[key] = ""
                            
                            if isinstance(chunk_content[key], list):
                                if isinstance(combined_content[key], list):
                                    combined_content[key].extend(chunk_content[key])
                            elif isinstance(chunk_content[key], dict):
                                if isinstance(combined_content[key], dict):
                                    combined_content[key].update(chunk_content[key])
                            else:
                                combined_content[key] = chunk_content[key]
            ...
            # Make sure we have the total count in the summary
            combined_content["total_names_count"] = len(all_generations)
    ...
    def _format_section_content(
        ...
        if section_type == "executive_summary":
            ...
            # Log that we're formatting the survey simulation section with detailed info
            logger.info(f"Formatting survey simulation section with data keys: {list(section_data.keys())}")
            if "survey_simulations" in section_data:
                logger.info(f"Survey simulations found: {len(section_data['survey_simulations'])}")
                if len(section_data['survey_simulations']) > 0:
                    first_sim = section_data['survey_simulations'][0]
                    logger.info(f"First simulation keys: {list(first_sim.keys() if isinstance(first_sim, dict) else [])}")
            
            # First ensure we have a summary from the raw content
            if "summary" not in formatted_content and isinstance(section_data, str):
                formatted_content["summary"] = section_data
                logger.info("Added survey simulation summary from string data")
            elif "summary" not in formatted_content and isinstance(section_data, dict) and "survey_simulation" in section_data:
                formatted_content["summary"] = section_data["survey_simulation"]
                logger.info("Added survey simulation summary from survey_simulation key")
            
            # For survey simulation, create a comprehensive table
            formatted_content["table"] = {
                "headers": ["Brand Name", "Persona", "Company", "Role", "Brand Score", "Key Feedback"],
            ...
            # If we don't have a summary but have the raw survey simulation text from the notepad
            if "summary" not in formatted_content and "survey_simulation" in section_data:
                formatted_content["summary"] = section_data["survey_simulation"]
                logger.info("Added survey simulation summary from survey_simulation key")
                
            # Log the final content structure
            logger.info(f"Final survey simulation formatted content keys: {list(formatted_content.keys())}")
            if "table" in formatted_content:
                logger.info(f"Survey table has {len(formatted_content['table'].get('rows', []))} rows")
            if "summary" in formatted_content:
                logger.info(f"Survey summary length: {len(formatted_content['summary']) if isinstance(formatted_content.get('summary'), str) else 'not a string'}")
            ...
            if isinstance(section_data, dict) and "market_researches" in section_data:
                ...
                for research in researches:
                    if isinstance(research, dict):
                        ...
                        formatted_content["table"]["rows"].append(row)
        elif section_type == "linguistic_analysis":
            # Format linguistic analysis as structured data
            formatted_content["table"] = {
                "headers": ["Brand Name", "Pronunciation", "Rhythm & Meter", "Sound Symbolism", "Word Class", "Readability"],
                "rows": []  # Will be filled with actual data rows
            }
            
            # Attempt to extract linguistic analysis data
            if isinstance(section_data, dict) and "linguistic_analyses" in section_data:
                analyses = section_data["linguistic_analyses"]
                logger.info(f"Found {len(analyses)} linguistic analyses to format")
                
                for analysis in analyses:
                    if isinstance(analysis, dict):
                        # Log each analysis being processed
                        logger.info(f"Processing linguistic analysis for brand: {analysis.get('brand_name', 'Unknown')}")
                        
                        row = [
                            analysis.get("brand_name", ""),
                            analysis.get("pronunciation_ease", ""),
                            analysis.get("rhythm_and_meter", ""),
                            analysis.get("sound_symbolism", ""),
                            analysis.get("word_class", ""),
                            analysis.get("overall_readability_score", "")
                        ]
                        formatted_content["table"]["rows"].append(row)
                
                # Log the formatted table structure
                logger.info(f"Created linguistic analysis table with {len(formatted_content['table']['rows'])} rows")
            else:
                logger.warning("No linguistic_analyses key found in section_data or it's not a dictionary")
        else:
    ...
    async def _generate_document(self, report: Dict[str, Any], run_id: str, user_prompt: str = None) -> str:
        ...
        def _process_survey_simulation_section(content):
            ...
            if isinstance(content, dict):
                # Log all the keys to help diagnose issues
                logger.info(f"Survey simulation content keys: {list(content.keys())}")
                ...
                if "summary" in content:
                    summary_len = len(content["summary"]) if isinstance(content["summary"], str) else "not a string"
                    logger.info(f"Survey summary length: {summary_len}")
                    logger.info(f"Survey summary first 100 chars: {content['summary'][:100] if isinstance(content['summary'], str) else 'not a string'}")
            else:
                logger.warning(f"Survey simulation content is not a dictionary, it's {type(content)}")
